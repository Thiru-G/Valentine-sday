...
// 18 images (use .avif - must match actual filenames in game-photos)
const images = [
  "/game-photos/1.avif",
  "/game-photos/2.avif",
  "/game-photos/3.avif",
...
export default function PhotoPairGame({
  ...
  // Shuffle only on client after mount to avoid hydration mismatch (Math.random differs server vs client)
  useEffect(() => {
    setImages(shuffleArray([...imagePairs]));
  }, []);

  const handleClick = async (index: number) => {
    if (!images || selected.length === 2 || matched.includes(index)) return;
  ...
  useEffect(() => {
    if (images && matched.length === imagePairs.length) {
      handleShowProposal();
    }
  }, [images, matched, handleShowProposal]);

  // Render placeholder until client has shuffled (avoids hydration mismatch)
  if (!images) {
    return (
      <div className="grid grid-cols-9 gap-1 lg:gap-2 max-w-[95vw] mx-auto place-items-center">
        {heartLayout.flat().map((index, i) =>
          index !== null ? (
            <div
              key={i}
              className="w-[11vh] h-[11vh] lg:w-20 lg:h-20 bg-gray-300 rounded-sm lg:rounded-md"
            />
          ) : (
